---
title: "零代码平台 - 手机端"
description: 极智云零代码平台是企业数智化转型的敏捷引擎，集表单引擎、流程引擎、智能查询表、可视化仪表盘、自定义公式、组织架构管理、协同工作台及应用构建器等核心能力于一体。无需代码基础，即可快速搭建贴合业务需求的企业级应用，实现团队高效协同。大幅降低开发与运维成本，支持云端便捷服务与本地化灵活部署，助力企业轻松迈向智慧运营新时代
---

# 零代码平台 - 手机端
极智云零代码平台是企业数智化转型的敏捷引擎，集表单引擎、流程引擎、智能查询表、可视化仪表盘、自定义公式、组织架构管理、协同工作台及应用构建器等核心能力于一体。无需代码基础，即可快速搭建贴合业务需求的企业级应用，实现团队高效协同。大幅降低开发与运维成本，支持云端便捷服务与本地化灵活部署，助力企业轻松迈向智慧运营新时代
<br/>
<br/>
免费使用：  
https://joy.eintelli.cn/


## 1.工作台
<img src="./image/工作台.jpg" class="mobile-img"/>

## 2.表单
表单数据
<img src="./image/表单-数据.jpg" class="mobile-img"/>
  
表单编辑
<img src="./image/表单-编辑.jpg" class="mobile-img"/>
  
下拉框
<img src="./image/表单-下拉框.jpg" class="mobile-img"/>
  
日期选择
<img src="./image/表单-日期.jpg" class="mobile-img"/>

## 3.仪表盘
<img src="./image/仪表盘-1.jpg" class="mobile-img"/>
<br/>
<img src="./image/仪表盘-2.jpg" class="mobile-img"/>

## 4.菜单
<img src="./image/应用菜单.jpg" class="mobile-img"/>

## 联系我们
如果您有软件需求或咨询，请联系我们，竭诚为您提供专业信息化服务。<br/>
联系电话：15928679486（祝先生）

微信二维码（请扫码添加）

![](/contact/wechat-qrcode-zj.png)


<style scoped>
@media (min-width: 450px) {
  .mobile-img {
    width: 360px;
  }
}
</style>